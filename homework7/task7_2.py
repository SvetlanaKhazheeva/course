...
class PersonInfo:
    ...
    def short_name(self):
        """
        Возвращает короткое имя: Фамилия И.
        """
    ...
    def new_salary(self):
        """
        Расчет новой зарплаты по формуле:
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        return: Новая зарплата
        """
        char = ''.join(self.departments).replace(' ', '')
        letters_dict = []
        while char != '':
            letters_dict.append(char.count(char[0]))
            char.replace(char[0], '')
        letters_dict.sort()
