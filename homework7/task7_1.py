...
class Segment:
    def __init__(self, start_point, end_point):
        '''
        Класс Отрезок
        :param start_point: начало отрезка с координатами точек (x1, y1)
        :param end_point: конец отрезка с координатами точек (x2, y2)
        '''
    ...
    def length(self):
        ...
        return: длина отрезка = math.sqrt((x2 - x1)**2 + (y2- y1)**2)
        """
        ln = round(((self.start_point[0] - self.end_point[0]) ** 2 +
                    (self.start_point[1] - self.end_point[1]) ** 2) ** 0.5, 2)
        return ln

    def x_axis_intersection(self):
        """
        Определяет пересечение отрезка с ось X
        return: True - пересекает, иначе False
        """
        if self.start_point[1] * self.end_point[1] <= 0:
            return True
        else:
            return False
    ...
    def y_axis_intersection(self):
        """
        Определяет пересечение отрезка с осю Y
        return: True - пересекает, иначе False
        """
