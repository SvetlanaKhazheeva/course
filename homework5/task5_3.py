...
def everything_for_your_cat(cats_data):
    """
    Преобразовывает списки кортежей в строку с оптимизацией ФИ владельца в одну строку
    :param cats_data: список кортежей
    :return: our_str - словарь, где ключ - имя фамилия владельца,значение - Кличка, Возраст котика
    """
    our_str = ""
    owners_cats = {}
    for a in cats_data:
        cat_name, cat_age, owner_name, owner_surname = a
        if (owner_name, owner_surname) not in owners_cats:
            owners_cats[(owner_name, owner_surname)] = [(cat_name, cat_age)]
        else:
            owners_cats[(owner_name, owner_surname)].append((cat_name, cat_age))
    for buyer, pets in owners_cats.items():
        pets_str = "; ".join([f"{pet[0]}, {pet[1]}" for pet in pets])
        our_str += f"{buyer[0]} {buyer[1]}: {pets_str}" "\n"
    return our_str
