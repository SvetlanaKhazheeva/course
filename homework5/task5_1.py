...
def letter_stat(our_str):
    """
    Принимает строку и возвращает в словарь,
    ключи - буквы заданного слова, значения - количество вхождений буквы в данное слово
    :param our_str: исходная строка
    :return: словарь
    """
    letters_dict = {} # создаем пустой словарь
    for a in range(len(our_str)):
        value = 0
        for b in range(len(our_str)):
            if our_str[b] == our_str[a]:
                value = value + 1  # находим количество вхождений буквы в заданное слово
        letters_dict.update({our_str[a]: value}) # объединение ключа и значения
    return letters_dict
